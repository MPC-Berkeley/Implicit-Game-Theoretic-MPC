...
# from casadi.casadi import vertsplit
import numpy as np
import scipy as sp
import casadi as ca
...
from mpclab_controllers.utils.controllerTypes import ALGAMESParams

from dataclasses import dataclass, field

class ALGAMES(AbstractController):
    ...
    def solve(self, states: List[VehicleState], 
        ...
        for i in range(self.outer_iters):
            ...
            if self.verbose:
                self.print_method('===================================================')
                self.print_method(f'ALGAMES iteration: {i}')
            ...
            for j in range(self.newton_iters):
                ...
                if opt_vio < self.opt_tol:
                    if self.verbose:
                        self.print_method(f' - Newton iteration: {j} | stat: {opt_vio:.4e} | converged: Gradient of Lagrangian within specified tolerance')
                ...
                # H = self.f_H(*ca.horzsplit(q_bar, 1), 
                #             *ca.horzsplit(u_bar, 1),
                #             *ca.horzsplit(mu_bar, 1),
                #             lam_bar,
                #             rho_bar,
                #             q_reg_it,
                #             u_reg_it)
                # self.print_method(f'Stationarity: {opt_vio}')
                # self.print_method(f'Residual: {norm_Gy}')
                # self.print_method(f'Residual jacobian: {np.linalg.norm(H, 1)}')
                # pdb.set_trace()
                ...
                for k in range(self.line_search_iters):
                    ...
                    if self.verbose:
                        self.print_method(f'   - Line search iteration: {k} | LS G norm: {norm_Gy_trial:.4e} | G norm: {norm_Gy:.4e} | a: {alpha:.4e}')
                ...
                if not line_search_converged:
                    if self.verbose:
                        self.print_method('   - Max line search iterations reached, did not converge')
                        self.print_method(f' - Newton iteration: {j} | Line search did not converge')
                    newton_converged = False
                ...
                if self.verbose:
                    self.print_method(f' - Newton iteration: {j} | G norm: {np.linalg.norm(Gy_bar, ord=np.inf):.4e} | step size: {d:.4e} | reg: {u_reg_it:.4e}')
                # pdb.set_trace()

                # Check for convergence
                if d < self.newton_step_tol:
                    if self.verbose:
                        self.print_method(f' - Newton iteration: {j} | converged: Average step size within specified tolerance')
                    newton_converged = True
            ...
            if newton_solves == self.newton_iters:
                ...
                if self.verbose:
                    self.print_method(f' - Newton iteration: {j} | Max Newton iterations reached, did not converge')
            ...
            if self.verbose:
                self.print_method(f'ALGAMES iteration: {i} | ineq vio: {max_ineq_vio:.4e} | eq vio: {max_eq_vio:.4e} | comp vio: {comp:.4e} | opt vio: {max_opt_vio:.4e}')
            ...
            if max_ineq_vio < self.ineq_tol \
                ...
                if self.verbose:
                    self.print_method('ALGAMES iterations converged within specified tolerances')
                    self.print_method('===================================================')
            ...
            if np.linalg.norm(u_bar[:,:-1].toarray().ravel()-u_im1[:,:-1].toarray().ravel()) < self.opt_tol/2 \
                ...
                if rel_tol_its >= self.rel_tol_req and max_ineq_vio < self.ineq_tol and max_eq_vio < self.eq_tol:
                    ...
                    converged = True
                    msg = 'conv_rel_tol'
                    if self.verbose: self.print_method('ALGAMES iterations converged via relative tolerance')
                    break
            else:
                rel_tol_its = 0
            ...
            if max_opt_vio > 1e5:
                ...
                iter_data.append(dict(cond=cond,
                                ...
                                m_sol=copy.copy(mu_bar)))
                if self.verbose:
                    self.print_method('ALGAMES diverged')
                    self.print_method('===================================================')
                msg = 'diverged'
                converged = False
                break
            ...
            it_dur = (datetime.now()-it_start).total_seconds()
            if self.verbose:
                self.print_method(f'ALGAMES iteration time: {it_dur}')
        ...
        if not converged and i == self.outer_iters-1:
            if self.verbose:
                # self.print_method('Max ALGAMES iterations reached, did not converge, using best solution from iter %i' % self.best_iter)
                self.print_method('Max ALGAMES iterations reached, did not converge')
                self.print_method('===================================================')
            msg = 'max_it'
            self.q_pred = copy.copy(q_bar.toarray().T)
            self.u_pred = copy.copy(u_bar[:,:-1].toarray().T)
        ...
        self.print_method(f'Solve iters: {i+1}')
    ...
    def _build_solver(self):
        ...
        if self.code_gen and not self.jit:
            ...
            os.chdir(gen_path)
            if self.verbose:
                self.print_method('- Generating C code for solver %s at %s' % (self.solver_name, str(gen_path)))
            generator.generate()
            pdb.set_trace()
            # Compile into shared object
            so_path = gen_path.joinpath(self.so_file_name)
            if self.verbose:
                self.print_method('- Compiling shared object %s from %s' % (so_path, c_path))
                self.print_method('- Executing "gcc -fPIC -shared -%s %s -o %s"' % (self.opt_flag, c_path, so_path))
    ...
    def _load_solver(self, solver_path=None):
        ...
        if self.verbose:
            self.print_method('- Loading solver from %s' % str(solver_path))
