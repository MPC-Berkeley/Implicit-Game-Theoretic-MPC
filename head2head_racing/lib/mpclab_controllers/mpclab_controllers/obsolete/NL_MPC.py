...
from mpclab_controllers.utils.controllerTypes import NLMPCParams


class NL_MPC(AbstractController):
    def __init__(self, dynamics, racetrack, control_params=NLMPCParams(), print_method=print):
    ...
    def solve(self, state, x_ref = None, input_prev=None):
        ...
        if exitflag == 1:
            ...
            x_ws = self.x_pred[1:]
            u_ws = self.u_pred[1:]
            x_ws = np.vstack((x_ws, np.array(self.dynamics.fd(x_ws[-1], u_ws[-1])).squeeze()))
    ...
    def _load_solver(self, solver_dir):
        ...
        self.print_method('Solver loaded')
    ...
    def _build_solver(self):
        ...
        def stage_equality_constraint(z, p):
            ...
            return ca.vertcat(self.dynamics.fd(q, u), u)

        def terminal_equality_constraint(z, p):
            q = z[:self.n]
            u = z[self.n:self.n + self.d]

            return self.dynamics.fd(q, u)
        ...
        # Generate solver (this may take a while for hiself.print_method('Solver generation complete')gh optlevel)
        self.model.generate_solver(self.options)
        self.print_method('Solver generation complete')
...
if __name__ == "__main__":
    ...
    from mpclab_common.pytypes import VehicleConfig, VehicleState, VehicleActuation
    ...
    solver_dir = os.path.expanduser('~/.mpclab_controllers/NL_MPC_solver_forces_pro')
    if not os.path.exists(solver_dir):
        print('Solver does not yet exist, creating...')
        solver_dir = ''

    params = NLMPCParams(n=n, d=d, N=N,
    ...
    # pdb.set_trace()
    nl_mpc = NL_MPC(dynamics, racetrack, params)
    # pdb.set_trace()
    x_0 = np.random.rand(n)
    x_ref = np.random.rand(N + 1, n)
    u_prev = np.random.rand(d)
    x_ws = np.random.rand(N + 1, n)
    u_ws = np.random.rand(N, d)
    ...
    nl_mpc.initialize()
    print('LTV MPC Controller instantiated successfully')

    nl_mpc.set_warm_start(x_ws, u_ws)
    u, preds, info = nl_mpc.solve(state, x_ref, input_prev)
    pdb.set_trace()
