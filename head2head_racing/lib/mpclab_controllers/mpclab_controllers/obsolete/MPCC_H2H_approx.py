...
from mpclab_controllers.utils.controllerTypes import MPCCFullModelParams, MPCCApproxFullModelParams
...
class MPCC_H2H_approx(AbstractController):
    ...
    def step(self, ego_state: VehicleState, tv_state: VehicleState, tv_pred: VehiclePrediction = None,
        ...
        contains_parametric = tv_pred and tv_pred.s and np.any(tv_pred.s)
        contains_global = tv_pred and tv_pred.x and np.any(tv_pred.x)
        gp_pred = None
        ...
        if tv_pred is not None and tv_pred.t and tv_state is not None and tv_state.t:
            ...
            #     offs += 1
            #     t_ += self.dt
            if contains_parametric:
                offs = np.argmin([abs(tv_state.p.s - s_) for s_ in tv_pred.s])
                difference = tv_state.p.s - tv_pred.s[offs]
            elif contains_global:
                offs = np.argmin([(tv_state.x.x - x)**2 + (tv_state.x.y - y)**2 for (x,y) in zip(tv_pred.x, tv_pred.y)])
            # TODO: renable when using non-ROS environment
            tv_pred.xy_cov = np.array(tv_pred.xy_cov).reshape(self.N, 4)
            if contains_parametric and contains_global:
                ...
                for i, (s, x_tran, e_psi) in enumerate(zip(tv_pred.s[offs:self.N], tv_pred.x_tran[offs:self.N],
                    ...
                    xy_std = tv_pred.xy_cov[offs:][i] if tv_pred.xy_cov is not None else np.zeros((2, 2))
                    xy_std = xy_std.reshape(2, 2)
                    global_coord = self.track.local_to_global((s + difference, x_tran, e_psi))
                    obstacle[i] = RectangleObstacle(xc=global_coord[0], yc=global_coord[1], psi=global_coord[2],
                                                        s=s + difference, x_tran=x_tran,
                                                        h=self.lencar, w=self.widthcar,
                                                        std_local_x=self.num_std_deviations * np.sqrt(xy_std[0, 0]),
                                                        std_local_y=self.num_std_deviations * np.sqrt(xy_std[1, 1]))
            elif contains_global:
        ...
        if logger is not None:
            logger(f"x_ref current: {x_ref}")
            logger(f"xref_scale: {xref_scale}")
            logger(f"blocking: {blocking}")
            logger(f"obs: {obstacle[2].xc}")
            logger(f"obs2: {obstacle[2].h}")
    ...
    def _build_solver(self):
        def nonlinear_ineq(z, p):
            ...
            if self.slack:
                ...
                slack_l_hard = ca.if_else(step > -1, 0.4 * (self.lencar - self.lencar * obstacle_slack_e / 10), 0)
                slack_w_hard = ca.if_else(step > -1, 0.4 * (self.widthcar - self.widthcar * obstacle_slack_e / 10), 0)
        ...
        if self.slack:
            self.model.ub = np.concatenate(
                (self.state_ub, [self.state_ub[self.zvars.index('s')]], self.input_ub, self.input_ub, np.array([1]), np.array([
                    10])))  # [x, u, u_prev, obs_slack, track_slack, obs_slack_e] track slack is constrained t 100 to avoid INF
