...
class iLQR_MA_RHC(AbstractController):
    ...
    def solve(self, vehicle_states: List[VehicleState]):
        ...
        for i in range(self.max_iters):
            ...
            #     s += ' | improvement, update accepted, lowering reg'
            ...
            #         s += ' | no improvement but cost sequence converged, update accepted, not changing reg'
            #         q_nom = copy.copy(q_new)
            #         u_nom = ca.horzcat(u_new, ca.DM(self.u_prev))
            #         cost_nom = copy.copy(cost)
            #     else:
            #         s = ' | no improvement, update rejected, increasing reg'
    ...
    def _build_solver(self):
        ...
        for k in range(self.N-1, -1, -1):
            ...
            if self.ddp:
                Dqq_f_k = list(self.joint_dynamics.fEd(q_nom[k], u_nom[k]))
                Duu_f_k = list(self.joint_dynamics.fFd(q_nom[k], u_nom[k]))
                Duq_f_k = list(self.joint_dynamics.fGd(q_nom[k], u_nom[k]))
            ...
            for i in range(self.M):
                ...
                args_Q = C_out + V_out[i] + dyn_args + [q_reg]
