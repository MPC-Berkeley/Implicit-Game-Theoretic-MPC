...
class DGSQP(AbstractController):
    def __init__(self, joint_dynamics: CasadiDecoupledMultiAgentDynamicsModel, 
        ...
        if self.qp_interface == 'casadi':
            ...
            # # Restoration phase QP solver
            # self.rp_solver = ca.conic('rp_qp', self.qp_solver, {'h': Qrp_sparsity, 'a': Grp_sparsity}, solver_opts)
        ...
        try:
            if self.qp_interface == 'casadi':
                ...
                if self.solver.stats()['success']:
                    ...
                    # if self.verbose: self.print_method('QP solution unsuccessful, solving elastic version')
                    # Qem = self.f_Qem(Q)
                    # qem = self.f_qem(q, self.eta)
                    # Gem = self.f_Du_Cem(G)
                    # gem = self.f_gem(-g)
                    # em_sol = self.em_solver(h=Qem, g=qem, a=Gem, uba=gem, x0=np.concatenate((x0, np.zeros(np.sum(self.n_c)))))
                    # em_status = self.em_solver.stats()['return_status']
                    # if self.verbose: self.print_method(f'QP status: {em_status}')
                    # if self.em_solver.stats()['success']:
                    #     du = em_sol['x'].toarray().squeeze()[:-np.sum(self.n_c)]
                    #     dl = np.zeros(np.sum(self.n_c))
                    #     # slack = em_sol['x'].toarray().squeeze()[-np.sum(self.n_c):]
                    #     # l_slack = em_sol['lam_a'].toarray().squeeze()[-np.sum(self.n_c):]
                    #     # pdb.set_trace()
                    # else:
                    #     du = [None]
                    #     dl = [None]
    ...
    # def _solve_restoration_qp(self, Q, q, G, g, x0=None):
    #     t = time.time()
    #     if x0 is None:
    #         x0 = np.zeros(self.N*self.n_u)
    ...
    #     try:
    #         if self.qp_interface == 'casadi':
    #             sol = self.rp_solver(h=Q, g=q, a=G, uba=-g, x0=np.concatenate((x0, np.zeros(np.sum(self.n_c)))))
    #             status = self.solver.stats()['return_status']
    #             if self.verbose: self.print_method(f'QP status: {status}')
    #             if self.solver.stats()['success']:
    #                 du = sol['x'].toarray().squeeze()[:-np.sum(self.n_c)]
    #             else:
    #                 du = [None]
    #         else:
    #             raise(ValueError(f'Unsupported QP interface {self.qp_interface}'))
    ...
    def solve(self, states: List[VehicleState], 
        ...
        self.nms_memory = deque([self.nms_initial_reference_factor*float(self.f_phi(l, np.maximum(0, g), q, G, g, 0))], self.nms_memory_size)
        ...
        while True:
            ...
            # self._check_licq(u, l, x0, up, parameters)
            # x_bar = np.array(self.evaluate_dynamics(u, x0)).squeeze()
            # s_waypoints = self.joint_dynamics.dynamics_models[0].track.key_pts[:,3]
            # s1 = x_bar[:,4]
            # s2 = x_bar[:,10]
            # D = []
            # for s in s1:
            #     idx = np.argmin(np.abs(s_waypoints-s))
            #     D.append(s - s_waypoints[idx])
            # print(D)
            ...
            if m_step:
                ...
                _phi = float(self.f_phi(_l, _s, _q, _G, _g, mu)) # Evaluate merit function at relaxed step
                ...
                if _phi <= (1-self.sigma)*R:
                    ...
                    #     _primal_feasibility = checkpoint_data.primal_feasibility
                    #     __G = checkpoint_data.qp['G']
                    # else:
                    #     _primal_feasibility = _data.primal_feasibility
                    #     __G = G_i
                    ...
                    for i in range(self.line_search_iters):
                        # if _a < self.alpha_min:
                        #     self.print_method('-- Minimum step size reached, switching to restoration mode')
                        ...
                        # if self.soc and i == 0:
                        #     _vio_m = max(0, np.amax(_g)) # Constraint violation
                        #     if _vio_m > _primal_feasibility:
                        #         if self.verbose: self.print_method(f'- Performing second order correction')
                        #         success = False
                        #         # _g_soc = _g - qp['G'] @ (_a*du)
                        #         _g_soc = _g
                        #         for j in range(self.soc_steps):
                        #             _du_soc, _ = self._solve_qp(np.eye(len(du)), np.zeros(len(du)), __G, _g_soc, _l) # Min norm
                        #             qp_solves += 1
                        #             # Corrected step
                        #             _du_cor = _a*du + _du_soc
                        #             _u_soc = u + _du_cor
                        #             _q_soc, _G_soc, _g_soc, _ = self._evaluate(_u_soc, _l, x0, up, P=parameters, hessian=False)
                        #             _s_soc = np.maximum(0, _g_soc)
                        #             _vio = np.amax(_s_soc)
                        #             _phi = float(self.f_phi(_l, _s_soc, _q_soc, _G_soc, _g_soc, mu))
                        #             if self.verbose: self.print_method(f'-- Second order correction step: {j} | constr vio: {_vio_m:.3E} -> {_vio:.3E} | merit : {_phi:.3f}')
                        #             # Check merit decrease
                        #             if _phi <= (1-self.sigma)*R:
                        #                 success = True
                        #                 _data.primal_step = _du_cor
                        #                 _data.step_size = _a
                        #                 _data.qp['g'] = _g_soc
                        #                 if self.verbose: self.print_method('-- Second order correction step resulted in merit function decrease')
                        #                 break
                        #             # Stop if constraint violation increases
                        #             if _vio > 0.99*_vio_m:
                        #                 if self.verbose: self.print_method('-- Second order correction step resulted in constraint violation increase')
                        #                 break
                        #             _vio_m = copy.copy(_vio)
                        #         if success:
                        #             _u = _u_soc
                        #             conv = True
                        #             break
                        ...
                        if self.verbose:
                            self.print_method(f'-- Line search step: {i} | merit : {_phi:.3e} | reference: {R:.3e} | a: {_a:.3e}')
                    ...
                    # if self.restoration:
                    #     # Evaluate restoration QP
                    #     qp = checkpoint_data.qp

                    #     _Q_rp = self.f_Qrp(self.xi)
                    #     _q_rp = self.f_qrp(self.rho)
                    #     _G_rp = self.f_Du_Crp(qp['G'])
                    #     _g_rp = self.f_grp(qp['g'])

                    #     _du_rp = self._solve_restoration_qp(_Q_rp, _q_rp, _G_rp, _g_rp)
                    #     qp_solves += 1
                    #     self.restoration = False
                    #     if None in du:
                    #         sqp_converged = False
                    ...
                    #     # _phi_rp = float(self.f_phi(_l, _s, _q, _G, _g, mu))

                    #     if self.verbose:
                    #         _vio_m = max(0, np.amax(qp['g']))
                    #         _vio = max(0, np.amax(ca.vertcat(*self.f_C(_u, x0, up, parameters))))
                    #         self.print_method(f'-- Restoration step | constr vio: {_vio_m:.3E} -> {_vio:.3E}')
    ...
    def _build_solver(self):
        ...
        # # Restoration phase
        # rho_ph = self.ca_sym('rho', 1)
        # qrp = ca.vertcat(ca.DM.zeros(n), rho_ph*ca.DM.ones(np.sum(self.n_c), 1))
        # self.f_qrp = ca.Function('f_q', [rho_ph], [qrp], self.options)

        # xi_ph = self.ca_sym('xi', 1)
        # Qrp = self.ca_sym('Qrp', ca.Sparsity(n+np.sum(self.n_c), n+np.sum(self.n_c)))
        # Qrp[:n,:n] = xi_ph*ca.DM.eye(n)
        # Qrp[n:,n:] = 1e-6*ca.DM.eye(np.sum(self.n_c))
        # self.f_Qrp = ca.Function('f_Qrp', [xi_ph], [Qrp])

        # grp = ca.vertcat(g_ph, ca.DM.zeros(np.sum(self.n_c), 1))
        # self.f_grp = ca.Function('f_grp', [g_ph], [grp], self.options)

        # Du_Crp = self.ca_sym('Du_Crp', ca.Sparsity(2*np.sum(self.n_c), n+np.sum(self.n_c)))
        # Du_Crp[:np.sum(self.n_c),:n] = H_ph
        # Du_Crp[:np.sum(self.n_c),n:] = -ca.DM.eye(np.sum(self.n_c))
        # Du_Crp[np.sum(self.n_c):,n:] = -ca.DM.eye(np.sum(self.n_c))
        # self.f_Du_Crp = ca.Function('f_Du_Crp', [H_ph], [Du_Crp], self.options)
    ...
    def _check_licq(self, u, l, x0, up, P):
        _, _G, _, _ = self._evaluate(u, l, x0, up, P=P, hessian=False)
        _active = np.where(l > ZERO_TOL)[0]
        _Ga = np.array(_G[_active,:]) # Gradient of active constraints
        _r = len(_active) # Number of active constraints
        if _r == 0:
            _rk = 0
        else:
            _rk = np.linalg.matrix_rank(_Ga)
        self.print_method(f'Active constraint matrix: {_Ga.shape} | rank: {_rk}')
        # self.print_method(f'n_x: {len(u)}')
        # self.print_method(f'n_g (active): {_r}, matrix rank: {_rk}')
        if _r == _rk:
            self.print_method(f'LICQ satisfied')
            return True
        else:
            self.print_method(f'LICQ not satisfied')
            return False

    # def check_second_order_optimality(self, z, l, v):
    #     y = np.concatenate((z, l))
    #     _active = np.where(l > ZERO_TOL)[0]
    #     Dg = np.array(dgdz(v, y)[_active,:]) # Gradient of active constraints
    #     Tg = la.null_space(Dg) # Tangent space of active constraints gradients
    #     D2L = d2L(v, y) # Hessian of Lagrangian
    #     _eigs = np.linalg.eig(Tg.T @ D2L @ Tg)[0]
    #     print(f'Subspace Lagrangian Hessian Eigenvalues: {_eigs}')
    #     if np.all(_eigs > ZERO_TOL):
    #         return True
    #     else:
    #         return False
    ...
    #         if self.verbose:
    #             self.print_method(f'- Line search iteration: {i} | merit gap: {phi_trial-(phi + self.beta*_a*dphi):.4e} | a: {_a:.4e}')
    #         if phi_trial <= phi + self.beta*_a*dphi:
    #             conv = True
    #             break
    #         else:
    #             _a *= self.tau
