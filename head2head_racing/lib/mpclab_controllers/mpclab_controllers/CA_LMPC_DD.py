...
class CA_LMPC_DD(AbstractController):
    def __init__(self, dynamics: CasadiDynamicsModel, 
        ...
        if self.code_gen and self.jit:
            self.options = dict(jit=True, jit_name='LMPC_DD', compiler='shell', jit_options=dict(compiler='gcc', flags=['-%s' % self.opt_flag], verbose=self.verbose))
        ...
        for k in range(self.N):
            ...
            if self.reg_state_out_idxs:
                for i in range(len(self.reg_state_out_idxs)):
                    ...
                    qk_reg = qk[ii].T
                    uk_reg = uk[ii].T
                    num_pts = qk_reg.shape[1]
                    qk_reg = ca.DM(qk_reg)
                    uk_reg = ca.DM(uk_reg)
                    batch_dynamics = self.dynamics.fd.map(num_pts)
                    qkp1_pred = batch_dynamics(qk_reg, uk_reg).full()[:, :num_pts].T

                    reg_Y_data = qkp1[:,self.reg_state_out_idxs[i]][ii] - qkp1_pred[:, self.reg_state_out_idxs[i]]
                    ...
                    dA = R[:len(self.reg_state_in_idxs[i])]
                    dB = R[len(self.reg_state_in_idxs[i]):-1]
                    dg = R[-1]
                    
                    _A[self.reg_state_out_idxs[i],self.reg_state_in_idxs[i]] += dA
                    _B[self.reg_state_out_idxs[i],self.reg_input_in_idxs[i]] += dB
                    _g[self.reg_state_out_idxs[i]] += dg
    ...
    def _compute_nn_indices(self, z, data, weights, h, max_pts=25):
        ...
        #     idxs_ret = _idxs[:min(max_pts, len(_idxs))]
        # else:
        #     idxs_ret = idxs[:min(max_pts, len(idxs))]
