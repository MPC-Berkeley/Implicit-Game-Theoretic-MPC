...
class DGSQP(AbstractController):
    ...
    def solve(self, states: List[VehicleState]):
        ...
        while True:
            ...
            if sqp_it == 0 or self.hessian_approximation == 'none':
                ...
                q_i, G_i, g_i, _ = self._evaluate(u, l, x0, up, hessian=False)
                def jac_eval(u, l):
                    q, G, _, _ = self._evaluate(u, l, x0, up, hessian=False)
                    return [q + G.T @ l]
                # Approximate Hessian
    ...
    def _build_solver(self):
        ...
        self.f_Du_L = ca.Function('f_Du_L', [ca.vertcat(*ua_ph), l_ph, xr_ph[0], uk_ph[-1]], Du_L)
        Duu_L = [ca.jacobian(Du_L[a], ca.vertcat(*ua_ph)) for a in range(self.M)]
        self.f_Duu_L = ca.Function('f_Duu_L', [ca.vertcat(*ua_ph), l_ph, xr_ph[0], uk_ph[-1]], Duu_L)
    ...
    def _watchdog_line_search_4(self, u_k, du_k, l_k, dl_k, s_k, ds_k, Q_k, q_k, G_k, g_k, evaluate, merit, d_merit):
        if self.verbose:
            self.print_method('===================================================')
            self.print_method('Watchdog step acceptance routine')
        qp_solves = 0
