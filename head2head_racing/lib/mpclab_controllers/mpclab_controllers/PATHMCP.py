...
from mpclab_controllers.utils.controllerTypes import PATHMCPParams

class PATHMCP(AbstractController):
    ...
    def _build_solver(self):
        ...
        # self.f_Ca = [ca.Function('f_Ca', [ca.vertcat(*x_ph), ca.vertcat(*ua_ph), uk_ph[-1], P], [ca.vertcat(*Ca[a])]) for a in range(self.M)]
        # self.f_Cs = ca.Function('f_Cs', [ca.vertcat(*x_ph), ca.vertcat(*ua_ph), uk_ph[-1], P], [ca.vertcat(*Cs)])
        # _Ca = [self.f_Ca[a](ca.vertcat(*xr_ph), ca.vertcat(*ua_ph), uk_ph[-1], P) for a in range(self.M)]
        # _Cs = self.f_Cs(ca.vertcat(*xr_ph), ca.vertcat(*ua_ph), uk_ph[-1], P)
        # _C = ca.vertcat(_Cs, *_Ca)

        # la_ph = [self.ca_sym(f'l{a}_ph', _Ca[a].shape[0]) for a in range(self.M)]
        # ls_ph = self.ca_sym('ls_ph', _Cs.shape[0])

        # _q = ca.vertcat(*[ca.jacobian(Ju[a], ua_ph[a]).T for a in range(self.M)])
        # _G = ca.jacobian(_C, ca.vertcat(*ua_ph))
        # self.f_lws = ca.Function('f_lws', [ca.vertcat(*ua_ph), xr_ph[0], uk_ph[-1], P], [_q, _G])

        # L = [Ju[a] + ca.dot(ls_ph, _Cs) + ca.dot(la_ph[a], _Ca[a]) for a in range(self.M)]
        # _Du_L = [ca.jacobian(L[a], ua_ph[a]).T for a in range(self.M)]
        
        # F = ca.vertcat(*_Du_L, -_C)
        # self.F = ca.Function('F', [ca.vertcat(*ua_ph, ls_ph, *la_ph), xr_ph[0], uk_ph[-1], P], [F])
        # J = ca.jacobian(F, ca.vertcat(*ua_ph, l_ph))
        # self.J = ca.Function('J', [ca.vertcat(*ua_ph, ls_ph, *la_ph), xr_ph[0], uk_ph[-1], P], [J])
