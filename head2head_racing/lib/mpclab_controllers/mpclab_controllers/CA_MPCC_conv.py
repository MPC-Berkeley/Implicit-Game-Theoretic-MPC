...
class CA_MPCC_conv(AbstractController):
    def __init__(self, dynamics: CasadiDynamicsModel, 
        ...
        if self.debug_plot:
            ...
            self.track.plot_map(self.ax_xy, close_loop=False)
    ...
    def _update_debug_plot(self, q, s, u, v) -> None:
    ...
    def _evaluate_dynamics(self, q0: np.ndarray, s0: np.ndarray, U: np.ndarray, VS: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        t = time.time()
        Q, S = [q0], [s0]
    ...
    def _build_solver(self) -> None:
        if isinstance(self.track, RadiusArclengthTrack):
            ...
            for s in S:
                ...
                Y.append(y)
                # Inside boundary
                xi, yi, _ = self.track.local_to_global((s, float(self.track.left_width(s))-self.track_tightening, 0))
                Xi.append(xi)
                Yi.append(yi)
                # Outside boundary
                xo, yo, _ = self.track.local_to_global((s, -(float(self.track.right_width(s))-self.track_tightening), 0))
                Xo.append(xo)
                Yo.append(yo)
            ...
            self.dsdx_s = self.track.dx
            self.dsdy_s = self.track.dy
        else:
            raise(ValueError(f'Track of type {type(self.track)} is not supported'))

        # Dynamics augmented with arc length dynamics
        ...
        if self.soft_track:
            track_slack_ph = ca.MX.sym('track_slack_ph', 1)
        ...
        T_xw = ca.DM(sp.sparse.diags(self.xw_scaling))
        T_xw_inv = ca.DM(sp.sparse.diags(self.xw_scaling_inv))
        ...
        if self.qp_interface == 'casadi':
            ...
            # To use cplex with casadi==3.5.5.:
            ...
            # Install CPLEX using sudo (this will install to /opt/ibm/ILOG)
            # create a symbolic link 'ln -s /opt/ibm/ILOG/CPLEX_Studio221/cplex/bin/x86-64_linux/libcplex2210.so libcplex1263.so ' 
            # in ~/.local/lib/python3.8/site-packages/casadi (for local installation)
            # in /usr/local/lib/python3.8/site-packages/casadi (for global installation)
            solver = 'cplex'
    ...
    def _solve_casadi(self, D: np.ndarray, P: np.ndarray) -> Tuple[np.ndarray, bool, str]:
        ...
        if self.soft_track:
            ...
            D_scaling = np.concatenate((D_scaling, np.ones(1)))
            ub = np.concatenate((ub, np.inf*np.ones(1)))
            lb = np.concatenate((lb, -np.inf*np.ones(1)))
        ...
        if success:
            t = time.time()
            D_bar = D_scaling_inv*np.array(sol['x']).squeeze()
            if self.soft_track:
                slacks = D_bar[-1]
                D_bar = D_bar[:-1]
                print(slacks)
    ...
    def _solve_cvxpy(self, D: np.ndarray, P: np.ndarray) -> Tuple[np.ndarray, bool, str]:
        ...
        for k in range(self.N+1):
            ...
            if k > 0:
                ...
                if Cxw[k].size1() > 0:
                    if self.soft_track:
                        constraints += [Cxw[k].full()[0] @ x[:,k] + s <= ubCxw[k].full()[0], Cxw[k].full()[0] @ x[:,k] + s >= lbCxw[k].full()[0]]
                        if Cxw[k].size1() > 1:
        ...
        if self.soft_track:
            objective += 0.5*cp.quad_form(s, self.q_tq) + self.q_tl * s
